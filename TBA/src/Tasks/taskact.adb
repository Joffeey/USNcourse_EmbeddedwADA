...
package body TaskAct is
   ...
   begin
      ...
      loop
         ...
         if Elapsed_CPU_Act > HighestCPU_Act then
            HighestCPU_Act := Elapsed_CPU_Act;
            Put_Line ("Highest CPU time in ACT as of now: " & To_Duration (HighestCPU_Act)'Image & " seconds");
            Put_Line ("Stopwatch time in ACT while highest CPU: " & To_Duration (Elapsed_Stopwatch_Act)'Image & " seconds");
         end if;
